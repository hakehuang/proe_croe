...
#endif
IMPLEMENT_DYNAMIC(CCreoMFCDialog,CDialog)

CCreoMFCDialog::CCreoMFCDialog(UINT nID, CWnd* pParent /*=NULL*/)
	: CDialog(nID, pParent)
	, m_iClientWidth(0)
	, m_iClientHeight(0)
	, m_iMinWidth(0)
...
CCreoMFCDialog::~CCreoMFCDialog()
{
	UnRegisterToolkitDlg();
}

BEGIN_MESSAGE_MAP(CCreoMFCDialog, CDialog)
	ON_WM_SIZE()
	ON_WM_SIZING()
END_MESSAGE_MAP()
BOOL CCreoMFCDialog::OnInitDialog()
{
...
void CCreoMFCDialog::OnSize(UINT nType, int cx, int cy)
{
...
void CCreoMFCDialog::OnSizing(UINT nSide, LPRECT lpRect)
{
...
BOOL CCreoMFCDialog::SetControlProperty(PDLGCTLINFO lp, int nElements)
{
	//  设置控件数组信息
...
void CCreoMFCDialog::ShowSizeIcon(BOOL bShow /*=NULL*/)
{
	m_bShowSizeIcon = bShow;
}

void CCreoMFCDialog::OnCancel()
{
...
void CCreoMFCDialog::PostNcDestroy()
{
	// TODO: 在此添加专用代码和/或调用基类
	CString msg = CString(GetRuntimeClass()->m_lpszClassName);
	CDialog::PostNcDestroy();
	delete this;
}

void CCreoMFCDialog::RegisterToolkitDlg()
{
// 	g_pToolkitDlgs.push_back(this);
}

void CCreoMFCDialog::UnRegisterToolkitDlg()
{
// 	g_pToolkitDlgs.erase(remove(g_pToolkitDlgs.begin(), g_pToolkitDlgs.end(), this), g_pToolkitDlgs.end());
}
